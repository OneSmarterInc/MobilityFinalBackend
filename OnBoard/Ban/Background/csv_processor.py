...
def process_csv_from_buffer(instance, buffer_data):
    print(buffer_data)
    print("Processing CSV data from buffer")
    csv_path = buffer_data.get('csv_path')
    company = buffer_data.get('company')
    vendor = buffer_data.get('vendor')
    account_number = buffer_data.get('account_number')
    sub_company = buffer_data.get('sub_company')
    mapping_data = buffer_data.get('mapping_json')
    entryType = buffer_data.get('entry_type')

    location = buffer_data.get('location')
    master_account = buffer_data.get('master_account')

    if csv_path:
        print("csv_path: " + csv_path)
        process_csv_data(instance, csv_path,account_number,company,vendor,sub_company,mapping_data)
    elif buffer_data.get('location_csv_path'):
        loc_csv_path = buffer_data.get('location_csv_path')
        process_location_csv_data(loc_csv_path)
    elif buffer_data.get('excel_csv_path'):
        ex_path = buffer_data.get('excel_csv_path')

        # process_excel_csv_data(ex_path,account_number,company,vendor,sub_company,mapping_data,entryType,location,master_account)
        process_excel_csv_data(instance, ex_path, account_number, company, vendor, sub_company, mapping_data, entryType,location,master_account)

def process_csv_data(instance, csv_path,acc_no,company,vendor,sub_company,mapping_data):
    ...
    latest_entry_dict =  json.loads(json.dumps(mapping_data))
...
# Fetch distinct values using Django ORM
    ...
    try:
        ...
        if company in company_list:
            print('in1')
            if sub_company in sub_company_list:
                print('in2')
        ...
        if acc_no in unique_account_numbers_list:
            checked_acc = acc_no
        else:
            checked_acc = None

    df_csv['sub_company'] = sub_company
    ...
    for _, row in df_csv.iterrows():
        ...
        if not UniquePdfDataTable.objects.filter(wireless_number=wireless_number).exists():
            UniquePdfDataTable.objects.create(inventory=instance, **row.to_dict())
            print("Data added to UniquePdfDataTable")


def process_location_csv_data(csv_path):
...
def process_excel_csv_data(instance, csv_path,acc_no,company,vendor,sub_company,mapping_data,entryType,location,master_account):
    ...
    from ..models import BaseDataTable, UniquePdfDataTable
    ...
    def process_data(mapping_data, df_csv, company, vendor, sub_company, acc_no, entryType, location, master_account):
        ...
        BaseDataTable.objects.create(inventory=instance, **b_dict)
    ...
    process_data(mapping_data, df_csv, company, vendor, sub_company, acc_no, entryType, location, master_account)
