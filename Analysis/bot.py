...
def get_database(db_path="db.sqlite3", query_type=None):
    ...
    elif query_type == "output_file":
        allowed_tables = {"AnalysisData"}
    else:
        allowed_tables = {"AnalysisData", "Analysis_summarydata"}
...
def inject_analysis_filter(query: str, analysis_id: int, chatType: str) -> str:
    query = query.strip().rstrip(";")
    lowered = query.lower()

    column = "analysis_id" if chatType == "single" else "multiple_analysis_id"

    # If the query already mentions the filter column, skip injection
    if f"{column.lower()} =" in lowered:
        return query

    # If there's already a WHERE clause → append with AND
    if " where " in lowered:
        return re.sub(r"\bwhere\b", f"WHERE", query, count=1) + f" AND {column} = {analysis_id}"
    else:
        # No WHERE clause at all → add one
        return f"{query} WHERE {column} = {analysis_id}"




def execute_sql_query(conn, query,chatType, analysis_id):
    try:
        query = clean_sql_query(query).strip()
        query = inject_analysis_filter(query, analysis_id, chatType=chatType)
        print(query)
...
def main():
    ...
    while True:
        ...
        result_df = execute_sql_query(db_connection, sql_query, analysis_id=2)
        print(result_df)
