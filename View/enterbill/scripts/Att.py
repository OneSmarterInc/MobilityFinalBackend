...
class Att:
    def __init__(self, input_file):
        ...
        if num_of_pages < 999:
            ...
            matches2 = re.findall(pattern2, total_text)
            # print('matactctch',matches)
            # print('balooo',matches2)
            self.plan_price_list = []
            if matches:
    ...
    def data_manage_part1(self):
        ...
        return Lines,merged_all_usage_df
    ...
    def geo_members(self):
        ...
        for i in range(lines_length):
            ...
            if match:
                ...
                try:
                    if 'Usage summary' in lines[i+2]:
                        try:
                            num_check = float(line[-2:])
                            if num_check.is_integer():
                                if 'Group' in line:
                                    wireless_pattern = r'\b\d{3}[-.\s]?\d{3}[-.\s]?\d{4}\b'
                                    match = re.search(wireless_pattern, lines[i+4])
                                    wireless_number = match.group() if match else None
                                    current_section = 'Group Plan Cost'
                                    gro_name = line[-2:]
                                    gro_cost = 0
                                    group_dict = {'Group Wireless_number': wireless_number, 'Group Number': gro_name,
                                            'Group': gro_cost}
                                    gro_members.append(group_dict)
                        except:
                            pass
                except:
                    pass
            ...
            if current_section == ('Monthly charges' and check == 1) or (
                ...
                for gro_data in gro_members:
                    if gro_data.get('Group Wireless_number') == phone_number:
                        # print('php',phone_number,'group_no',gro_data.get('Group Wireless_number'))
...
def process_all(uploaded_file):
        ...
        if no_of_pages < 999:
            ...
            final_df = pd.merge(intital_df,plan_price_df,on='Group_Number',how='left')
            ...
            # final_df = final_df.concat(filtered_df[['User_name', 'Charges']], ignore_index=True)
            final_df = pd.concat([final_df, filtered_df[['User_name', 'Charges']]], ignore_index=True)
            ...
            final_df['Charges'] = final_df['Charges'].replace('[\$,]', '', regex=True).astype(float)
            final_df['Item_Description'] = final_df['Item_Description'].astype(str)
            final_df = final_df[~final_df['Item_Description'].str.contains('Plan_Allocation')]
        else:
            intital_df = data_list.to_csv(y)
            final_df = pd.merge(intital_df,plan_price_df,on='Group_Number',how='left')
            # unique_df = final_df.drop_duplicates(subset='Wireless_number')
            ...
            unique_df.drop(columns=['Messaging_Usage','Data Usage(GB)','Voice_Plan_Usage'])
            new_merged_summary_df = pd.merge(unique_df,yt,on='Wireless_number',how='left')
            new_merged_summary_df.rename(columns={
            'ECPD Profile ID': 'ECPD Profile ID',
            'Bill Cycle Date': 'Bill Cycle Date',
            'Wireless_number': 'Wireless Number',
            'User_name': 'User Name',
            'Page Number': 'Cost Center',
            'Date Due': 'Date Due',
            'Invoice Number': 'Invoice Number',
            'Bill Period': 'Bill Period',
            'Bill Cycle Date': 'Bill Cycle Date',
            'Activity since last bill': 'Account Charges and Credits',
            'Monthly Charges': 'Monthly Charges',
            'Equipment Charges': 'Equipment Charges',
            'Monthly charges Add-ons': 'Usage and Purchase Charges',
            'Surcharges and Credits': 'Total Surcharges and Other Charges and Credits',
            'Taxes, Governmental Surcharges and Fees': 'Taxes, Governmental Surcharges and Fees',
            'Billing_Name': 'Cost Center',  # This appears to be redundant, adjust as needed
            'Billing_Address': 'Billing_Address',  # Not listed in new names
            'Remidence_Addresss': 'Remidence_Addresss',  # Not listed in new names
            ...
            'Voice_Plan_Usage ' : 'Voice Plan Usage'
                }, inplace=True)
            def convert_to_gb(row):
                if row['Units'] == 'KB':
                    return float(row['Data Usage'].replace(',','')) / 1048576  # Convert KB to GB
                elif row['Units'] == 'MB':
                    return float(row['Data Usage'].replace(',','')) / 1024  # Convert MB to GB
                else:
                    return row['Data Usage']  # Leave unchanged if the unit is already in GB

            # Apply the conversion function to the DataFrame
            new_merged_summary_df['Data Usage (GB)'] = new_merged_summary_df.apply(convert_to_gb, axis=1)

            # Optionally, you can remove the 'Units' column if it's no longer needed
            new_merged_summary_df.drop('Units', axis=1, inplace=True)

            new_merged_summary_df = new_merged_summary_df[[
                'ECPD Profile ID', 
                'Bill Cycle Date', 
                'Account Number',           # Make sure this column exists
                'Date Due', 
                'Invoice Number', 
                'Bill Period', 
                'Wireless Number', 
                ...
                'Cost Center', 
                'User ID', 
                'Your Calling Plan',        # Ensure this column is mapped correctly
                'Account Charges and Credits', 
                'Monthly Charges', 
                'Usage and Purchase Charges', 
                'Equipment Charges', 
                'Total Surcharges and Other Charges and Credits', 
                'Taxes, Governmental Surcharges and Fees', 
                'Third Party Charges',      # Make sure this column exists or is mapped
                'Total Charges', 
                'Voice Plan Usage', 
                'Messaging Usage', 
                'Data Usage (KB)', 
            ...
            unique_df = new_merged_summary_df
