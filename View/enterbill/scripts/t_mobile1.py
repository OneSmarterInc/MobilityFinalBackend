...
def extract_data_tmobile_type2(t_mobile_filename):
    print("def extract_data_tmobile_type2")
    ...
    def extract_first_page_t_mobile_data(filename):
        print("def extract_first_page_t_mobile_data")
        with pdfplumber.open(filename) as pdf:
...
def get_all_dataframes(file):
    print("def get_all_dataframes")
    gtr_df,merged_df_base_data,merged_df_total_data,merged_df_temp_data,next_df,dtr_req,mmt,summer,usage= extract_data_tmobile_type2(file)
    output_dir = 'output_fileser'
    os.makedirs(output_dir, exist_ok=True)
    def format_wireless_number(parts):
        print("def format_wireless_number")
    ...
    filtered_df.to_csv(f'{output_dir}/xlsx_gottrew.csv', index=False)
    gtr_df.to_csv(f'{output_dir}/ xlsx_got2.csv', index=False)
    ...
    final_merged_df = final_merged_df[final_merged_df['item description'] != 'Total Charges']
    # final_merged_df.to_csv(f'{output_dir}/ xlsx_DETAILED_new.csv', index=False)
    # first_merged_df.to_csv(f'{output_dir}/ xlsx_SUMMARY_new.csv', index=False)
    descrepancies.to_csv(f'{output_dir}/ xlsx_Faultyxyxzzz_news.csv',index=False)
    # merged_df_temp_data.to_csv(f'{output_dir}/detailed_data_.csv',index=False)
    # merged_df_base_data.to_csv(f'{output_dir}/base_data_.csv', index=False)
    # merged_df_total_data.to_csv(f'{output_dir}/total_data_.csv', index=False)
    return summer,first_merged_df,final_merged_df,descrepancies,usage