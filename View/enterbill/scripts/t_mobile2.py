...
def extract_text_from_t_mobile(filename):
    print("def extract_text_from_t_mobile")
    def find_pattern_on_page(page, pattern):
        print("def find_pattern_on_page")
    ...
    def extract_text_by_position(file_path, pattern):
        print("def extract_text_by_position")
        data = ''
        pattern_found = False
    ...
    df1.to_csv(filename.replace('.pdf', '_Details.csv'), index=False)
    ...
    existing_columns = new_merged.columns
    ...
    # Select columns that exist in new_merged
    pdf_columns = [col for col in rename_mapping_pdf.keys() if col in existing_columns]
    base_columns = [col for col in rename_mapping_base.keys() if col in existing_columns]

    # Create DataFrames only with available columns
    pdf_df = new_merged[['Account Number', 'Wireless Number', 'Item Category', 
                        'Item Description', 'Charges', 'Plan', 'User Name_x', 
                        'Data', 'Total']].filter(items=existing_columns)

    base_data_df = new_merged[['Account Number', 'Address', 'Invoice Number', 
                            'Bill Cycle Date']].filter(items=existing_columns)

    # Take only the first row for base_data_df
    base_data_df = base_data_df.head(1)

    # Rename only existing columns
    pdf_df.rename(columns={k: v for k, v in rename_mapping_pdf.items() if k in pdf_columns}, inplace=True)
    base_data_df.rename(columns={k: v for k, v in rename_mapping_base.items() if k in base_columns}, inplace=True)

    rename_dict = {
        'account_number': 'accountnumber',
        'invoice_number':'invoicenumber'
    }
    # Convert column names to lowercase and replace spaces with dots
    ...
    # Additional renaming for base_data_df
    base_data_df.rename(columns={'account_number': 'accountnumber', 'invoice_number': 'invoicenumber'}, errors='ignore', inplace=True)

    # Ensure unique_df exists before modifying 'Total'
    if 'Total' in unique_df.columns:
        unique_df['Total'] = unique_df['Total'].replace('[\$,]', '', regex=True).astype(float)
